...
namespace VFolders
{
    public static class VFolders
    {
        static void RowGUI(string guid, Rect rowRect)
        {
            ...
            var isPackagesRoot = rowRect.x == 16 && !isFavoritesRoot && !isAssetsRoot;
            ...
            void drawing()
            {
                ...
                void customIcon()
                {
                    ...
                    var icon = useClearerRows ? EditorGUIUtility.IconContent(folderInfo.iconNameOrPath).image as Texture2D
                ...
                void contentMinimap()
                {
                    ...
                    {
                        if (!contentIcons_byName.TryGetValue(name, out var icon))
                            icon = contentIcons_byName[name] = EditorGUIUtility.IconContent(name).image as Texture2D;
        ...
        static Dictionary<string, Texture2D> contentIcons_byName = new Dictionary<string, Texture2D>();
        ...
        public static Texture2D GetSmallFolderIcon(FolderInfo folderInfo)
        {
            ...
            void generateAndCache()
            {
                ...
                void insertAssetIcon()
                {
                    ...
                    // assetIconOriginal.Resize()

                    assetIconOriginal.filterMode = prevFilter;


                    // var assetIcon = (EditorGUIUtility.IconContent(folderData.assetIconName).image as Texture2D).Resize_(assetIconSize, assetIconSize, true);
                    // var assetIconPixels = assetIcon.GetPixels();
        ...
        public static void DrawBigFolderIcon(Rect rect, FolderInfo folderInfo)
        {
            ...
            void assetIcon()
            {
                ...
                void icon()
                {
                    // SetGUIColor(isSelected ? Color.Lerp(GUI.skin.settings.selectionColor, Color.white, .8f) : Color.white);

                    GUI.DrawTexture(assetIconRect, EditorGUIUtility.IconContent(folderInfo.iconNameOrPath).image);
